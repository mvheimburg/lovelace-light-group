...
const valid = {
  ...
  sections: [
    { name: "Kjøkken", lights: [{ entity: "light.a", name: "My lamp" }] },
  ],
};
describe("config", () => {
  it("defaults empty configuration and preserves custom names without mutating", () => {
    ...
    result.sections[0].lights[0].name = "Changed";
  ...
  it.each([
    ...
    [],
    { type: "other" },
    { ...valid, appearance: "other" },
    { ...valid, color_scheme: "other" },
    { ...valid, sections: {} },
  ...
  it("keeps HA layout metadata", () =>
    expect(
      normalizeConfig({ ...valid, grid_options: { columns: 12 } }),
    ).toHaveProperty("grid_options", { columns: 12 }));
});
